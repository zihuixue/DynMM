...
def main():
    ...
    """
    model = ESANetOneModality(pretrained_on_imagenet=False, encoder='resnet34', encoder_block='NonBottleneck1D',
                              input_channels=1, num_classes=40,
                              nr_decoder_blocks=[3, 3, 3], channels_decoder=[512, 256, 128],
                              weighting_in_encoder='SE-add', upsampling='learned-3x3-zeropad')
    print(model)

    model.eval()
    rgb_image = torch.randn(1, 1, 480, 640)
    ...
    macs, params = profile(model, inputs=(inputs_rgb,))
    print(params / 10**6)
