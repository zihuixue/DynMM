...
def get_temp(start_epoch, curr_epoch, end_epoch, start_t, end_t):
    if curr_epoch <= start_epoch:
        return start_t
    if curr_epoch >= end_epoch:
        return end_t
    else:
        return start_t + (curr_epoch - start_epoch) / (end_epoch - start_epoch) * (end_t - start_t)


class ExpDecayTemp:
    def __init__(self, start_t, end_t, time_len):
        ...
        if time_len == 0:
            ...
            self.b = math.exp(1 / time_len * math.log(end_t / start_t))

    def get_t(self, epoch):
        return self.start_t * self.b ** epoch


def cmp_temp():
    exp = ExpDecayTemp(0.1, 0.001, 500)
    for e in range(500):
        t1 = get_temp(0, e, 500, 0.1, 0.001)
        t2 = exp.get_t(e)
        # temp_list.append(t)
        if e % 100 == 99:
            print(e, t1, t2)